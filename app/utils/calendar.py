...
def quarter_end_for_date(d: pd.Timestamp) -> pd.Timestamp:
    ...
    last_date = pd.Timestamp(year=d.year, month=q_end_month, day=1) + pd.offsets.MonthEnd(0)
    return last_date

def generate_quarter_dates(initial_date: date, end_date: date = DEFAULT_END_DATE) -> List[date]:
    """
